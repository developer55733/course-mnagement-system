...
// Notes Management Functions
  ...
  function initDiscussionManagement() {
    const addDiscussionForm = document.getElementById('addDiscussionForm');
    if (addDiscussionForm) {
      addDiscussionForm.addEventListener('submit', handleAddDiscussion);
    }
    ...
    if (discussions.length === 0) {
      discussionsList.innerHTML = `
        <div class="text-center text-muted py-4">
          ...
          <p>Use the form above to create the first discussion.</p>
        </div>
      `;
      return;
    }
          ...
          <div class="d-flex justify-content-between align-items-start">
            <div class="flex-grow-1">
              <h6 class="card-title mb-1">
                <i class="fas fa-comments text-primary"></i> ${discussion.title}
              </h6>
              <div class="text-muted small mb-2">
                <i class="fas fa-book"></i> ${discussion.module || 'General Discussion'} | 
                <i class="fas fa-user"></i> ${discussion.author} | 
                <i class="fas fa-clock"></i> ${new Date(discussion.created_at).toLocaleDateString()}
              </div>
              <p class="card-text">${discussion.content.substring(0, 150)}${discussion.content.length > 150 ? '...' : ''}</p>
            </div>
            <div class="ml-3">
              <button class="btn btn-sm btn-outline-primary" onclick="viewDiscussion(${discussion.id})">
                <i class="fas fa-eye"></i> View
              </button>
  ...
  window.viewDiscussion = (id) => {
    const discussion = discussions.find(d => d.id === id);
    if (discussion) {
      alert(`Discussion: ${discussion.title}\n\n${discussion.content}\n\nPosted by: ${discussion.author}\nModule: ${discussion.module || 'General Discussion'}`);
    }
  };

  // Initialize discussion management when DOM is ready
  document.addEventListener('DOMContentLoaded', function() {
